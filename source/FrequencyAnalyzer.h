...
public:
    Analyzer()
        : juce::Thread ("Frequency-Analyzer")
    {
        averager.clear();
    }
    ...
    void createPath (juce::Path& p, const juce::Rectangle<float> bounds, float minFreq) const
    {
        p.clear();
        ...
        for (int i = 0; i < averager.getNumSamples(); ++i)
            p.lineTo (bounds.getX() + factor * indexToX (float (i), minFreq),
...
private:
    ...
    juce::dsp::WindowingFunction<Type> windowing {
        size_t (fft.getSize()),
        juce::dsp::WindowingFunction<Type>::hann,
        true
    };
